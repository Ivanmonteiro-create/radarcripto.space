 ...
 * 1) Arquivo: app/simulador/page.tsx
 * 2) Instalar: npm i lightweight-charts
 * 3) Tailwind já deve estar configurado
 */
...
export default function RadarSimulatorPro() {
  ...
  useEffect(() => {
    ...
    ema9SeriesRef.current = chart.addLineSeries({ lineWidth: 2, color: "#60a5fa" });
    ema21SeriesRef.current = chart.addLineSeries({ lineWidth: 2, color: "#f59e0b" });
  ...
  function placeOrder(side: "BUY" | "SELL") {
    ...
    if (!position) {
      if (side === "BUY") {
        newPositions.push({ id: crypto.randomUUID(), symbol, side: "LONG", qty: qtyFromUSDT, entry: lastPrice, openedAt: Date.now() });
      } else {
        if (!allowShort) return;
        newPositions.push({ id: crypto.randomUUID(), symbol, side: "SHORT", qty: qtyFromUSDT, entry: lastPrice, openedAt: Date.now() });
      }
      ...
      if (side === dir) {
        ...
        setHistory((h) => [{ id: crypto.randomUUID(), symbol, side, price: lastPrice, qty: position.qty, pnl, at: Date.now() }, ...h]);
    ...
    setHistory((h) => [{ id: crypto.randomUUID(), symbol, side, price: lastPrice, qty: qtyFromUSDT, at: Date.now() }, ...h]);
    setBalance(newBalance);
    setPositions(newPositions);
  }
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <select className="rounded-xl bg-gray-800 px-3 py-2 text-sm" value={symbol} onChange={(e) => setSymbol(e.target.value)}>
            ...
            <select className="rounded-xl bg-gray-800 px-3 py-2 text-sm" value={timeframe} onChange={(e) => setTimeframe(e.target.value)}>
            ...
            <button onClick={resetAll} className="rounded-xl bg-gray-800 px-3 py-2 text-sm hover:bg-gray-700">
              Resetar
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
          <div className="lg:col-span-3 rounded-2xl border border-gray-800 bg-gray-900/50 p-3 shadow">
            <div className="mb-2 flex items-center justify-between">
              <div className="text-sm text-gray-300">{symbol.toUpperCase()}</div>
          ...
          <div className="rounded-2xl border border-gray-800 bg-gray-900/50 p-4 shadow">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className="w-full rounded-xl bg-gray-800 px-3 py-2 font-mono outline-none"
              />
            </div>

            <div className="mb-4 grid grid-cols-2 gap-2">
              <button onClick={() => placeOrder("BUY")} className="rounded-2xl bg-green-600 px-4 py-3 font-semibold shadow hover:bg-green-500">
                COMPRAR
              </button>
              <button onClick={() => placeOrder("SELL")} className="rounded-2xl bg-red-600 px-4 py-3 font-semibold shadow hover:bg-red-500">
                VENDER
              </button>
            </div>
            ...
            <div className="rounded-xl bg-gray-800 p-3">
              ...
              {!position ? (
                <div className="text-sm text-gray-400">Nenhuma posição em {symbol.toUpperCase()}</div>
              ) : (
                <div className="space-y-1 text-sm">
                  <div>Lado: {position.side}</div>
                  <div>Entrada: {position.entry.toFixed(2)}</div>
                  <div>Qtd: {position.qty.toFixed(6)}</div>
                  <div className={unrealized >= 0 ? "text-green-400" : "text-red-400"}>P&L: {formatUSD(unrealized)}</div>
                </div>
              )}
            </div>
          </div>
                ...
                <tbody>
                  {history.map((t) => (
                    <tr key={t.id} className="border-t border-gray-800">
                      ...
                      <td className={t.pnl === undefined ? "px-3 py-2 text-gray-400" : t.pnl >= 0 ? "px-3 py-2 text-green-400" : "
