 ...
 * 1) Crie um arquivo: app/simulador/page.tsx e exporte este componente por lá.
 * 2) Garanta que Tailwind esteja configurado. Este componente usa classes utilitárias.
 * 3) Não requer shadcn neste MVP. Apenas lightweight-charts (npm i lightweight-charts).
 */
...
interface Position {
  ...
  side: "LONG" | "SHORT"; // SHORT é opcional, pode desativar no UI
  qty: number; // em USDT convertidos para unidade do ativo (para simples: tratamos como unidades do ativo quando compra)
  entry: number; // preço médio
...
export default function RadarSimulatorPro() {
  ...
  useEffect(() => {
    ...
    const stream = `${symbol}@kline_${timeframe}`; // candle feed
    const trade = `${symbol}@trade`; // last price
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.stream.includes("@kline_")) {
          const k = data.data.k; // kline payload
          const bar: KlineBar = {
  ...
  useEffect(() => {
    ...
    const candle = chart.addCandlestickSeries({ upColor: "#16a34a", downColor: "#dc2626", borderDownColor: "#dc2626", borderUpColor: "#16a34a", wickDownColor: "#6b7280", wickUpColor: "#6b7280" });
    candleSeriesRef.current = candle;

    const ema9 = chart.addLineSeries({ lineWidth: 2 });
    const ema21 = chart.addLineSeries({ lineWidth: 2 });
  ...
  useEffect(() => {
    ...
    const ema9Data = ema9.map((v, i) => ({ time: bars[i].time as any, value: v }));
    const ema21Data = ema21.map((v, i) => ({ time: bars[i].time as any, value: v }));
  ...
  const unrealized = useMemo(() => {
    ...
    const qty = position.qty; // unidades do ativo
    return (lastPrice - position.entry) * qty * dir;
  }, [position, lastPrice]);

  function placeOrder(side: "BUY" | "SELL") {
    if (!lastPrice || orderSize <= 0) return;

    // Determina se é abertura ou fechamento
    // Simples: uma posição única por símbolo. BUY abre LONG; SELL fecha LONG ou abre SHORT se allowShort.
    let newPositions = [...positions];
    let newBalance = balance;
    const qtyFromUSDT = orderSize / lastPrice; // quantas unidades do ativo
    ...
    if (!position) {
      ...
      newBalance -= orderSize; // margem simples (didático)
    } else {
      // já existe posição — vamos tentar fechar (total) na direção oposta
      const dir = position.side === "LONG" ? "SELL" : "BUY";
      if (side === dir) {
        const pnl = unrealized;
        newBalance += orderSize + pnl; // devolve margem + P&L (modelo didático)
        newPositions = newPositions.filter((p) => p.symbol !== symbol);
        setHistory((h) => [
          { id: crypto.randomUUID(), symbol, side, price: lastPrice, qty: position.qty, pnl, at: Date.now() },
          ...h,
        ]);
        ...
        // mesma direção — aumenta posição (média de preço)
    ...
    setHistory((h) => [
      { id: crypto.randomUUID(), symbol, side, price: lastPrice, qty: qtyFromUSDT, at: Date.now() },
      ...h,
    ]);
    setBalance(newBalance);
    setPositions(newPositions);
            ...
            <div className="rounded-xl bg-gray-800 p-3">
              ...
              {!position ? (
                ...
                <div className="space-y-1 text-sm">
                  ...
                  <div className={"font-semibold " + (unrealized >= 0 ? "text-green-400" : "text-red-400")}>P&L: {formatUSD(unrealized)} ({pct(unrealized, orderSize).toFixed(2)}%)</div>
                </div>
              )}
            </div>
                ...
                <tbody>
                  {history.map((t) => (
                    <tr key={t.id} className="border-t border-gray-800">
                      ...
                      <td className={"px-3 py-2 font-mono " + (t.pnl === undefined ? "text-gray-400" : t.pnl >= 0 ? "text-green-400" : "text-red-400")}>{t.pnl === undefined ? "--" : formatUSD(t.pnl)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
